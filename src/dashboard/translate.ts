// Lightweight translation helper for the Prompt Testing playground. Lets an
// operator re-express an injection payload in any language Google Translate
// supports, so the same attack can be exercised against the detection pipeline
// in dozens of locales without leaving the dashboard.
//
// Uses Google's free, key-less `gtx` endpoint. It is unofficial and rate
// limited, which is fine for interactive single-prompt testing. The call is
// made server-side so the browser never hits a CORS wall.
...
// The full Google Translate language set (as of the v2 language list). `auto`
// is offered as the source-only sentinel for language auto-detection.
export const TRANSLATE_LANGUAGES: Language[] = [
  ...
  { code: 'ee', name: 'Ewe' }, { code: 'tl', name: 'Filipino' },
  ...
  { code: 'iw', name: 'Hebrew' }, { code: 'hi', name: 'Hindi' },
...
/**
 * Translate `text` into `target` using Google's free gtx endpoint.
 * `source` defaults to 'auto' (let Google detect the input language).
 * Throws on an unknown target code, a non-2xx response, or an unexpected body.
 */
export async function translateText(
  ...
  if (!SUPPORTED.has(target)) throw new Error(`unsupported target language: ${target}`)
  if (!text.trim()) return { translated: text, detectedSource: source }
  ...
  // gtx shape: [[["translated seg","src seg",…], …], …, "detectedLang", …]
  ...
  const translated = segments
    ...
    .join('')
  const detectedSource = typeof data[2] === 'string' ? data[2] : source
